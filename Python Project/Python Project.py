...
while not crashed:
	...
	if x < 0:
		...
		x = width - 40
#Logic for creating cars and liliypads and their movement
...
#Sets up play area and displays timer
	...
	gameClock(timer)
#Logic for lili pad and car movement
	if(y == 120):
		alive = False
	...
	if(not alive):
		if(lives <= 1):
			...
			direction = 0
			lives -= 1
#Spawning the lilipads and cars
	if lilyLanes[0] > 130:
	...
	for index, car in enumerate(cars3):
		gameDisplay.blit(pygame.image.load(car[2]), (car[0], car[1]))
		car[0] += 1
#Object deletion for lili pads and cars
