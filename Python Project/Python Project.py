...
#Group:Sam Ericksen, Domas Karvelis, Jeff Koopman
...
while not crashed:
	...
	if x < 0:
		...
		x = width - 40
#Logic for snapping Python character back to the grid after leaving lily pad
	...
	if y > height - 80:
		y = height - 80
#Sets up play area and displays timer
	...
	gameClock(timer)
#Logic for lily pad and car collision
	if(y == 120):
		alive = False
...
# Logic for losing lives.
	if(not alive):
		if(lives <= 1):
			...
			direction = 0
			lives -= 1
#Spawning the lily pads and cars
	if lilyLanes[0] > 130:
...
#Logic for lily and car movement.
	...
	for index, car in enumerate(cars3):
		gameDisplay.blit(pygame.image.load(car[2]), (car[0], car[1]))
		car[0] += 1
#Object deletion for lily pads and cars
	...
	if len(cars3) > 0 and cars3[0][0] > 800:
		cars3 = cars3[1:]
#Timer increment and Win condition
	timer += 1
